...
| `partners` | `id`, `name`, `kind` (`political` / `community`), `scope_state_id?` (FK `states`, `NULL` = nationwide ceiling), `code` (`[A-Z0-9]{2,6}`, unique, used in membership numbers), `logo_url?`, `status` (`active` / `inactive`), `created_by`, `created_at` | CR-0026 / ADR-0018. An affiliated organisation that recruits its own people. Read only by the super admin (`partners_super_all`) and by a partner's own `partner_admin` (`partners_own_select`). `name` and `scope_state_id` are editable (widen-only for the ceiling, `enforce_partner_ceiling_widen_only` trigger); `code` is fixed after onboarding. |
| `partnership_requests` | `id`, `name`, `organization`, `role_title?`, `email`, `phone?`, `message`, `status` (`new` / `contacted` / `onboarded` / `declined`), `partner_id?` (set on onboard), `handled_by?`, `handled_at?`, `created_at` | CR-0026 follow-up. Leads from the public partnership form. The anonymous form action writes here with the service role; `partnership_requests_super_all` (super admin only) is the sole policy, no public INSERT. |
